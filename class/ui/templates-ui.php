...
class TemplatesUI{
    ...
    public function __construct($db){
        ...
        // Check if user owns any templates
        $hasOwnedItems = false;
        $query = $_GET['query'] ?? "";
        $sort = $_GET['sort'] ?? 'updatedDate';
        $order = $_GET['order'] ?? 'DESC';

        $templateList = $this->db->fetchAllTemplate($query, $sort, $order);
        if ($templateList && strpos($templateList, 'edit-') !== false) {  // Check if any edit buttons exist
            $hasOwnedItems = true;
        }
                ...
                <div class="delete-container">
                    <div class="delete-confirmation">
                        ...
                        <div class="delete-content">
                            <b>Are you sure want to delete this template?</b>
                            <form method="POST">
...
HTML;
        ...
        $this->view .= <<<HTML
                    ...
                    <div class='template-title'>Checklist Template</div>
HTML;
        $this->view .= "<div class='button-container'><a href='index.php?view=viewTemplates&user=$user&id=0'><div class='create-template-button'><i class='fa-solid fa-plus fa-lg' style='color:#ffffff;margin-right:15px'></i>Create New Template</div></a></div></div>";
...
HTML;
        ...
        foreach ($columns as $key => $label) {
            ...
            $url = "index.php?view=templates&user=$user&query=$currentQuery&sort=$key&order=$nextOrder";
...
HTML;
    ...
    private function reconstructData($query){
        ...
        $sort = $_GET['sort'] ?? 'updatedDate';
        $order = $_GET['order'] ?? 'DESC';
        header("Location: index.php?view=templates&user=$user&query=$query&sort=$sort&order=$order&delete=");
    }
}
